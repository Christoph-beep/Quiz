...
func main() {
	...
	createAnswerFile()
	answerFileWrite("blau", 0)
	answerFileWrite("hund", 1)
...
func answerFileWrite(newSolutions string, counter int) {
	// reading part
	// check, if data has already been written to the file, therefore the file needs to be opened first

	// map for writing solutions into file
	// new
	// structure of maps : key[int], value[string]

	// just a dummy counter not working !!!

	type SolutionsQuiz struct {
		SolutionsMap map[int]string `json:"solutions"`
	}

	solutions := SolutionsQuiz{
		SolutionsMap: map[int]string{
			counter: newSolutions,
		},
	}
	fmt.Println("test")
	fmt.Println(solutions)

	jsonData, err := json.Marshal(solutions)
	...
	if size == 0 {
		// get number of curent elements within the map
		err2 := os.WriteFile(path, []byte(jsonData), 0644)
		if err2 != nil {
		...
		_, err = f.Write([]byte(append(jsonData)))
		if err != nil {
			log.Println(err)
		}
	}
